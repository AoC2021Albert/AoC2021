...
class SFN:
    ...
    def explode(self, n):
        if n==0:
            ...
            exploding = False
            if isinstance(self.left, SFN):
                exploding, newleft, spillleft, spillright =  self.left.explode(n-1)
                self.left = newleft
            ...
            if not exploding and isinstance(self.right, SFN):
                exploding, newright, spillleft, spillright =  self.right.explode(n-1)
                self.right = newright
    ...
    def split(self):
        ...
        if isinstance(self.left, SFN):
            ...
            if self.left > 9:
                self.left = SFN(StringReader("[" + str(self.left // 2) + "," + str((self.left +1 ) // 2) + "]"))
                splitting = True
        if not splitting:
            if isinstance(self.right, SFN):
                ...
                if self.right > 9:
                    self.right = SFN(StringReader("[" + str(self.right // 2) + "," + str((self.right +1 ) // 2 ) + "]"))
                    splitting = True
        return(splitting)
