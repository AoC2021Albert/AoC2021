  ...
  if memo[sparam]>costsofar:
    ...
    for fromroom in rooms.keys():
        if rooms[fromroom]:
            if rooms[fromroom] in ([fromroom],
                                   [fromroom,fromroom],
                                   [fromroom,fromroom,fromroom],
                                   [fromroom,fromroom,fromroom,fromroom]):
                #skip it, it is fine!
                ...
            else:
                ...
                if rooms[toroom] in [[],
                    ...
                    solve(newrooms,newcorridors,costsofar+distance*value[toroom])
                else:
                    corridorcandidates.append(fromroom)
    for i, c in enumerate(corridors):
        if c!="" and ( rooms[c] in [[],[c],[c,c],[c,c,c]]):
    ...
    for fromroom in corridorcandidates:
        for corridordestionation, distance in corridordestinations(fromroom, corridors):
            ...
            solve(newrooms,newcorridors,costsofar+distance*value[newcorridors[corridordestionation]])
